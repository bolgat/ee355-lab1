...
// New function to replace strcmp(),
// which skips non-alphanumeric characters
// when testing for equality
// Returns 0 if not equal, 1 if equal
int equal(char a[255], char b[255])
{
	// adj variables to skip
	// non-alphanumeric characters
	int a_adj=0, b_adj=0, idx=0;
	// Keep going until both words are finished
	while(a[idx+a_adj] != '\0' || b[idx+b_adj] != '\0')
	{
		// Flags for skipping a character in each word
		int a_skip = 0, b_skip = 0;
		// If a character is non-alphanumeric,
		// skip the character and turn on the skip flag.
		// isalnum from here: https://www.geeksforgeeks.org/isalnum-function-c-language/
		if(!isalnum(a[idx+a_adj]))
		{
			a_adj++;
			a_skip = 1;
		}
		if(!isalnum(b[idx+b_adj]))
		{
			b_adj++;
			b_skip = 1;
		}
		// If either word had a character skipped,
		// redo the loop without incrementing
		if(a_skip || b_skip) continue;
		// If no match, the words are not the same
		if(a[idx+a_adj] != b[idx+b_adj]) return 0;
		idx++;
	}
	// Check one more time if both words are the
	// same
	if(a[idx+a_adj] != b[idx+b_adj]) return 0;
	return 1;
}
...
int main ()
{
		...
		while (fscanf(infile, "%s", buff)!=EOF)
		{
			...
			// Check new word alongside each word
		  	for(int dict_idx = 0; dict_idx <= num_words; ++dict_idx)
		  	{
	        	...
	        	if(equal(buff, dict[dict_idx].word))
	        	{
	...
	outfile = fopen("out_improved.txt", "w+");
